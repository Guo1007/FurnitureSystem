...
- **用户系统** — 邮箱注册、密码登录、**验证码登录（仅邮箱）**、忘记密码重置、个人信息管理（改绑邮箱需新邮箱验证码、**昵称修改经 AI 审核 / 头像变更需管理员人工审核**）
- **家具浏览** — 首页推荐、分类浏览、商品详情、多图展示
- **购物车** — 游客可本地加购，结算需登录；数量修改、SKU 规格选择、抽屉内收货地址选择与添加、**结算选券抵扣**
- **订单系统** — 下单（**支持优惠券抵扣**）、**支付宝支付（沙箱电脑网站支付，预下单 + 异步回调自动确认 + 支付成功自动跳转）**、支付倒计时（24h）、超时自动取消、订单状态跟踪、**多状态页签筛选**、支付页/订单详情展示优惠明细
- **优惠券** — 领券中心（可领券/我的卡券，按类型筛选）、多类型券（满减/折扣/无门槛）、全场或按分类范围、新老用户人群、固定有效期或领取后 N 天；下单门槛/范围/**实时过期判定**，**多张券组合（支持"可叠加/不可叠加"规则：可叠加券可多张同用，不可叠加券只能单用）**，**取消·超时·退款自动归还**
...
- **AI 智能客服** — 独立对话页，基于 RAG 的家具推荐助手"小智"，支持流式对话、商品卡片推荐、**会话历史管理（侧栏历史列表、新建/重命名/删除、移动端抽屉）与仅保留近 3 天会话**
...
- **分类管理** — 家具类型维护
- **优惠券管理** — 券模板增删改查、启停，多类型/适用范围/人群/有效期配置，**可叠加/不可叠加开关**
- **订单管理** — 订单列表、状态流转、商品规格展示、**支付流水查看（渠道/交易号/金额/时间）**
- **售后处理** — 独立售后页，退款申请同意/拒绝/审核、库存释放
- **评价审核** — 评价/追评/回复的审核管理（通过/拒绝）
- **昵称/头像审核** — 用户昵称改动的 AI 审核、头像变更的人工审核（拒绝原因可下拉+自定义输入）
- **操作日志** — 管理端 CRUD 与用户端敏感操作留痕（参数脱敏、主键分页）
- **通知管理** — 站内通知发布、编辑、删除
- **通知设置** — 订单/退款/库存告警的邮件通知规则与接收人配置
- **内容配置** — 首页轮播图、服务栏等站点运营位管理
- **菜单分组** — 按商品/交易/用户/内容域分组，待处理角标提示（订单/退款/审核等实时刷新）
...
| **AI 能力** | LangChain4j · 通义千问 qwen3.7-flash（流式对话 + RAG 知识库检索）                 |
| **对象存储**  | 阿里云 OSS（家具图片上传）                                                     |
...
├── src/main/resources/
│   ├── knife4jDoc/         # Knife4j 首页 Markdown 文档（home.md）
│   └── content/            # AI RAG 知识库文档
├── sql/                    # 数据库脚本
│   └── furniture-system.sql           # 完整数据库结构 + 初始数据
├── packet-sniffer/         # HTTPS 加密验证工具（抓包对比 443 密文 / 80 明文，答辩演示用）
...
- `packet-sniffer/README` 无独立文档，`sniffer.py` 头部注释含使用说明（需管理员权限 + Npcap，用于验证 HTTPS 加密）
...
- **自定义首页**：`doc.html` 首页展示项目简介、功能模块、技术栈、接口约定、订单状态机等，内容来自 `src/main/resources/knife4jDoc/home.md`（Markdown，可直接编辑）
...
| `ALIPAY_APP_ID`       | 支付宝沙箱/正式应用 AppID         | 支付功能需     |
| `ALIPAY_APP_PRIVATE_KEY`| 支付宝应用私钥（签名）             | 支付功能需     |
| `ALIPAY_ALIPAY_PUBLIC_KEY`| 支付宝公钥（回调验签）            | 支付功能需     |
| `ALIPAY_NOTIFY_URL`   | 支付宝支付异步回调地址（公网 HTTPS）| 支付功能需     |
| `ALIPAY_GATEWAY`      | 支付宝网关（默认沙箱地址，已写死）    | 否           |
...
- **Token 认证**：自定义 Token 认证过滤器，Redis Hash 存储用户态，前后端分离无状态鉴权；**多端会话按端类型互踢（PC/移动端可并存，同端后者顶前者，被顶设备返回差异提示）**，改密/注销可一键清空该用户全部会话
- **优惠券**：领券用 **Redis Lua 原子计数**防并发超发、写库失败回滚计数；下单校验门槛/适用范围/采用 `expire_time` **实时过期判定**（不依赖懒纠正的状态位），**支持多张券组合（可叠加/不可叠加规则：可叠加券可多张同用、不可叠加券只能单用且不能与其它券共存，服务端校验）**，取消/超时/退款自动归还，`user_coupon` 只在读"我的券"时按 `expire_time` 懒纠正过期状态（仅影响展示，不影响抵扣安全）
- **支付宝支付**：沙箱/正式环境可切换，电脑网站支付，`pageExecute` 生成收银台表单；异步回调**验签(RSA2) + 幂等去重 + 金额核对**后仅由回调把订单置为已支付（前端轮询自动确认），支付流水台账可查
...
- **AI 会话管理**：会话历史按用户隔离存储本地（`aiChatMessages:u{userId}` 等），按会话分组管理（新建/重命名/删除），移动端抽屉切换，仅保留近 3 天记录
