...
- **订单系统** — 下单（**支持优惠券抵扣**）、支付倒计时（24h）、超时自动取消、订单状态跟踪、**多状态页签筛选**、支付页/订单详情展示优惠明细
- **优惠券** — 领券中心（可领券/我的卡券，按类型筛选）、多类型券（满减/折扣/无门槛）、全场或按分类范围、新老用户人群、固定有效期或领取后 N 天；下单门槛/范围/**实时过期判定**，**取消·超时·退款自动归还**
...
- **分类管理** — 家具类型维护
- **优惠券管理** — 券模板增删改查、启停，多类型/适用范围/人群/有效期配置
...
- **优惠券**：领券用 **Redis Lua 原子计数**防并发超发、写库失败回滚计数；下单校验门槛/适用范围/采用 `expire_time` **实时过期判定**（不依赖懒纠正的状态位），取消/超时/退款自动归还，`user_coupon` 只在读“我的券”时按 `expire_time` 懒纠正过期状态（仅影响展示，不影响抵扣安全）
